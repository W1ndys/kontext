...
// GeneratedContent 是 LLM 在生成阶段返回的配置文件内容。
type GeneratedContent struct {
	ProjectManifest string            `json:"project_manifest"`
	ArchitectureMap string            `json:"architecture_map"`
	Conventions     string            `json:"conventions"`
	ModuleContracts map[string]string `json:"module_contracts"` // 键为模块名（如 "cmd"），值为 JSON 内容
}
...
// SingleFileContent 是分步生成单个文件时的响应结构。
type SingleFileContent struct {
	Content string `json:"content"` // JSON 内容
}

// ModuleContractContent 是单个模块契约生成的响应结构。
type ModuleContractContent struct {
